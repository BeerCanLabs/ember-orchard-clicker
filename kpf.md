...
| Movie projector | A **one-time 10,000-ticket** shop upgrade that **unlocks at player level 15** and is required before any movie can be played; it does **not** change ticket earnings. | `the movie projector is a one-time 10000-ticket shop upgrade unlocked at level 15`, `the projector does not change ticket earnings` |
...
Before movies can be played the player must **buy the Movie projector** upgrade in the shop — a **one-time 10,000-ticket** purchase that appears at the bottom of the shop and unlocks at **player level 15**. Until it is owned, the bottom-right projector button is **hidden**.

Once owned, a fixed **retro-badge projector button** (flat design, built from an inline SVG glyph — no image assets) sits in the **bottom-right**: a solid orange pill with a two-reel projector glyph and a "MOVIE / PROJECTOR" label. Clicking it opens a **"Pick a movie"** prompt listing **two features** — the **Matinee** (10,000 tickets, ~1,000/s, **unlocks at level 15**) and the **Grand Premiere** (50,000 tickets, ~4,000/s, **unlocks at level 20**). A feature you haven't reached the level for shows a **🔒 dashed, locked card** and cannot be chosen. Choosing an unlocked one spends its cost and starts a showing.
